...
DROP POLICY IF EXISTS "Service role can insert conversation teams" ON conversation_teams;
CREATE POLICY "Service role can insert conversation teams"
    ON conversation_teams FOR INSERT
    WITH CHECK (true);
...
DROP POLICY IF EXISTS "Service role can insert AI analysis" ON message_ai_analysis;
CREATE POLICY "Service role can insert AI analysis"
    ON message_ai_analysis FOR INSERT
    WITH CHECK (true);
...
COMMENT ON TABLE teams IS 'Teams for organizing message routing (RH, Support, Sales, etc.)';
